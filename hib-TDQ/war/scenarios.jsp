...
<body class="no-sidebar">
	...
	<div class="wrapper wrapper-style3">
		<div class="title">${projectId.projectName}</div>
		<div class="container">
			<div class="row">
				<div class="12u">
					...
					.getNickname()%></h1>
										...
										<div class="actions">
											<form method="get" action="/scenarios">
												...
												<br>
												<div name="response">${noScens}</div>
												<input type="hidden" id="dispatchActionScenario"
													name="dispatchActionScenario" /> <input type="hidden"
													id="setId" name="setId" />
												<c:forEach items="${scenariosList}" var="scenarios">
													<div class="row">
														<div class="8u">
															${scenarios.scenarioName} <input type="submit"
																value="Delete scenario"
																class="button button-custom button-summary"
																onClick="setDispatchActionScenario('delete');setid('${scenarios.id}')" />
															<input type="submit" value="Show scenario details"
																class="button button-custom button-summary"
																onClick="setDispatchActionScenario('showDetails');setid('${scenarios.id}')" />
														</div>
													</div>
												</c:forEach>
										</div>
